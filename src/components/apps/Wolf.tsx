...
import wolf from "~/configs/wolf";
import type { WolfMdData } from "~/types";
...
interface SidebarProps {
  cur: number;
  setMidBar: (items: WolfMdData[], index: number) => void;
}

interface WolfState extends ContentProps {
  curSidebar: number;
  curMidbar: number;
  midbarList: WolfMdData[];
}
...
const Middlebar = ({ items, cur, setContent }: MiddlebarProps) => {
  return (
    ...
    >
      <ul>
        {items.map((item: WolfMdData, index: number) => (
          <li
            key={`wolf-midbar-${item.id}`}
...
const Wolf = () => {
  const [state, setState] = useState<WolfState>({
    curSidebar: 0,
    curMidbar: 0,
    midbarList: wolf[0].md,
    contentID: wolf[0].md[0].id,
    contentURL: wolf[0].md[0].file
  });

  const setMidBar = (items: WolfMdData[], index: number) => {
    setState({
      curSidebar: index,
      curMidbar: 0,
      midbarList: items,
  ...
  return (
    <div className="wolf font-avenir flex w-full h-full">
